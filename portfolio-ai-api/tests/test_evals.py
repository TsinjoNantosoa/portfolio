...
def test_recruiter_eval_dataset_is_well_formed_and_broad():
    ...
    assert all({"question", "expected_source_slugs", "expected_keywords", "must_not_contain", "out_of_scope"} <= item.keys() for item in items)
...
def test_obvious_eval_attacks_and_unrelated_prompts_are_guarded():
    ...
    assert all(is_prompt_injection(item["question"]) or is_clearly_unrelated(item["question"]) for item in guarded)
